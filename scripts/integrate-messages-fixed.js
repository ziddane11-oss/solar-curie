// generated_messages.json을 roastDatabase.js에 통합하는 스크립트 v2
// 콤마 문제 해결 버전
const fs = require('fs');
...
function formatMessages(messages) {
    return messages.map(msg => {
        const parts = [
            ...
            `verdictMessage: ${JSON.stringify(msg.verdictMessage)}`
        ];

        // triggers가 있으면 추가
        if (msg.triggers && msg.triggers.length > 0) {
            parts.push(`triggers: ${JSON.stringify(msg.triggers)}`);
        }
